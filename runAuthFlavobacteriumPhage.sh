...
if [[ "$RUN_KESTREL" -eq "1" ]]; then
  (time ./KESTREL $PARAM_KESTREL -o auth-fil-$REFERENCE-x.fq $REFERENCE.fa $TARGET ) &> REPORT-FLAVO;
fi
#==============================================================================
# RUN DAMAGE
if [[ "$RUN_DAMAGE" -eq "1" ]]; then
  ...
  bwa mem -t 8 -I 0 -O 2 -N 0.01 -L 1024 -E 7 $REFERENCE.fa auth-fil-$REFERENCE-x.fq > $REFERENCE.sam
